...
void main() {
  ...
  testWidgets('WelcomeScreen renders the identity and both entry points', (
    ...
    expect(find.widgetWithText(KarataButton, 'Create account'), findsOneWidget);
    expect(find.widgetWithText(KarataButton, 'Log in'), findsOneWidget);
  });

  testWidgets(
    'the debug backend picker (test runs are debug builds) defaults to the '
    'configured backend and can switch to a custom URL',
    (WidgetTester tester) async {
      ...
      // explicitly chosen.
      final line = find.textContaining('Debug backend: Cloud Run (karata0)');
      expect(line, findsOneWidget);
      expect(find.text('Server base URL'), findsNothing);

      // The line itself is the menu, rather than a separate icon beside it.
